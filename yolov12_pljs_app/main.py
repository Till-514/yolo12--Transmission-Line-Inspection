...
from PySide6.QtWidgets import (QApplication, QMainWindow, QWidget, QVBoxLayout,
                               ...
                               QGroupBox, QLineEdit, QInputDialog, QMessageBox,
                               QFormLayout, QTableWidget, QTableWidgetItem,
                               QHeaderView, QScrollArea, QDialog, QGridLayout)
...
# ==================== 替换 QtFusion.handlers.MediaHandler ====================
class MediaHandler(QThread):
...
class MainWindow(QMainWindow):
    ...
    def create_settings_button(self):
        ...
        # 创建AI分析按钮
        self.ai_analysis_btn = QPushButton("Deepseek")
    ...
    """)
        ...
        # AI分析组
        aianalyzer_group = QGroupBox("Deepseek分析")
        aianalyzer_layout = QVBoxLayout(aianalyzer_group)

        # 添加Deepseek分析控制按钮
        self.aianalyzer_start_btn = QPushButton("进行Deepseek分析")
        self.aianalyzer_stop_btn = QPushButton("发送分析报告")
    ...
    def create_right_panel(self):
        ...
        # 显示标题
        title = QLabel("天轨智寻——基于YOLOv12和 Deepseek分析的输电线路部件检测系统")
        title.setStyleSheet("""
            font-size: 22px;
            font-weight: bold;
            color: #00e5ff;
    ...
    def process_frame(self, image):
        ...
        if det is not None and len(det):
            ...
            for info in det_info:
                ...
                image = drawRectBox(image, bbox, label=label, alpha=0.2, addText=True, color=self.colors[cls_id])
        ...
        if det_info:
            ...
            for info in det_info:
                n = info['class_name']
                c = info['score']
                result_text += f"{n} {c*100:.1f}%  "
            # 把结果写到界面标签
            self.result_label.setText(result_text)
        else:
            # 无检测结果时的提示
            self.result_label.setText("🔍 未检测到输电线路部件")
